...
const DashboardHomePage = () => {
  ...
  // Generic function to update a todo item in state
  ...
  const renderHabitIcons = () => {
      // Find today's logs from the state
  ...
  const renderTodoList = () => {
    if (isLoading && todos.length === 0) return <div style={{display:'flex', justifyContent:'center', marginTop:'2rem'}}><LoadingSpinner /></div>; // Show spinner only if list is empty initially
          ...
          return (
            <div key={item.id} className={`${styles.todoItem} ${itemIsLoading ? styles.loading : ''}`}>
              <div className={styles.todoContent}>
                  {item.type === 'sleep' && (
                      <>
                          <p className={styles.todoText}>
                              {item.task_type === 'sleep' ? 'Go to Sleep' : 'Wake Up'}
                          </p>
                  ...
                  {item.type === 'hydrate' && (
                      <>
                          <p className={styles.todoText}>Drink Water</p>
                   ...
                   {item.type === 'diet' && (
                      <>
                          <p className={styles.todoText}>Log Meals</p>
                          <p className={styles.todoSubtext}>
